...
protocol PhotosServiceProtocol: AnyObject {
    var photoAssets: [PHAsset] { get }
    var videoAssets: [PHAsset] { get }
    ...
    func fetchImage(
        _ asset: PHAsset,
        size: CGSize,
        completion: @escaping (Image, PHAsset) -> Void
    )
}

final class PhotoVideoService: PhotosServiceProtocol {
    ...
    func fetchImage(
        ...
        manager.requestImage(
            ...
            guard let image else {
                completion(Image(uiImage: UIImage()), asset)
                return
            }
            completion(Image(uiImage: image), asset)
        }
    }    
}
...
//}
//
//class PhotosUseCaseImpl: BaseUsecase {
//    
...
//}
//
//extension PhotosUseCaseImpl: PhotosUseCase {
//    
//    func requestAcess(completion: @escaping (Bool) -> Void) {
//        PHPhotoLibrary.requestAuthorization(for: .readWrite) { status in
//            switch status {
...
//        ) { image, _ in
//            guard let image else {
//                completion(.init(asset: asset, image: UIImage()))
//                return
//            }
//            completion(.init(asset: asset, image: image))
//        }
//    }
//}
